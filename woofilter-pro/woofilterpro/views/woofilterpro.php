...
class WoofilterProViewWpf extends WoofiltersViewWpf {
	...
	public function generatePriceFilterHtml( $filter, $filterSettings, $blockStyle, $key = 1, $viewId = '' ) {
		...
		$settings['minValue'] = ReqWpf::getVar('min_price', 'all', -1);
		$settings['maxValue'] = ReqWpf::getVar('max_price', 'all', -1);
		...
		$html =
			'<div id="' . self::$blockId .
				...
				'" data-get-attribute="min_price,max_price,tax" data-decimal="' . $prices->decimal .
	...
	public function generateRatingFilterHtml( $filter, $filterSettings, $blockStyle, $key = 1, $viewId = '' ) {
		...
		$addText = esc_attr__($this->getFilterSetting($settings, 'f_add_text', 'and up'), 'woo-product-filter');
		$addText5 = esc_attr__($this->getFilterSetting($settings, 'f_add_text5', '5 only'), 'woo-product-filter');
	...
	public function generateAttributeFilterHtml( $filter, $filterSettings, $blockStyle, $key = 1, $viewId = '' ) {
		...
		list($showedTerms, $countsTerms, $showFilter, $allTerms) = $this->getShowedTerms($attrName, $showAllAtts);
	...
	public function generateAttributeSliderFilterHtml( $filter, $filterSettings, $blockStyle, $key = 1, $viewId = '' ) {
		...
		$showAllAtts = $this->getFilterSetting($settings, 'f_show_all_attributes', false);
		list($showedTerms, $countsTerms, $showFilter, $showedTermsWithotFiltering) = $this->getShowedTerms($attrName, $showAllAtts);
		...
		foreach ($showedTermsWithotFiltering as $id) {
			if (isset($termArr[$id])) {
				...
				if ( $showAllAtts || is_array( $showedTerms ) && in_array( $id, $showedTerms ) ) {
					if ( ! empty( $excludeIds ) && in_array( $id, $excludeIds ) ) {
						continue;
					}
					if ( $isForceNumeric ) {
...
/*		if ( is_array( $showedTerms ) && $isForceNumeric && 1 == count( $showedTerms ) && false === $settings['minValue'] && false === $settings['maxValue'] ) {
		...
		$disableNumberFormatting = ( $this->getFilterSetting($settings, 'f_disable_number_formatting', false) ) ? 'data-prettify-enabled = "false"' : '';
		$html .=
			$this->generateFilterHeaderHtml($filter, $filterSettings, $noActive) .
			$this->generateDescriptionHtml($filter) .
	...
	public function generateInStockFilterHtml( $filter, $filterSettings, $blockStyle, $key = 1, $viewId = '' ) {
		...
		$htmlOpt = '<option value="' . esc_html($defaultStockStatus) . '" data-slug="' . esc_html($defaultStockStatus) . '" selected>' . esc_attr($optionsAll[$defaultStockStatus]) . '</option>';
	...
	public function getProductAttrFilterData( $filter, $filterSettings, $key, $showedTerms = array() ) {
		...
		$productAttr = $isCustom ? DispatcherWpf::applyFilters('getCustomTerms', array(), $attrSlug, array_merge($args, array('wpf_fbv' => $this->getFilterSetting($filterSettings['settings'], 'filtering_by_variations')))) : $this->getTaxonomyHierarchy($taxonomyName, $args);
	...
	public function generateBrandFilterHtml( $filter, $filterSettings, $blockStyle, $key = 1, $viewId = '' ) {
		...
		$filterName = ( 'multi' === $type ) ? 'product_brand_list' : 'product_brand';
	...
	public function addCustomCss( $settings, $filterId ) {
		...
		$useHideButtonStyles = $this->getFilterSetting($settings, 'use_hide_button_styles', false);
		if ( !$useBlockStyles && !$useTitleStyles && !$useButtonStyles && !$useHideButtonStyles) {
			return '';
		}
