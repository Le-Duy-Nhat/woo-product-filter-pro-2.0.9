...
class WoofilterProWpf extends ModuleWpf {
	...
	public $acfEnabledTypes = array('text', 'number', 'radio', 'select', 'button_group', 'checkbox', 'true_false', 'date_picker', 'date_time_picker', 'time_picker', 'color_picker');
	...
	public function init() {
		...
		DispatcherWpf::addFilter('getColorGroup', array( $this, 'getColorGroup' ), 10, 3);
		DispatcherWpf::addFilter('getColorParents', array( $this, 'getColorParents' ), 10, 2);
	...
	public function showVariationAsMineImage( $image, $product = null ) {
			...
			if ( false !== $varTable ) {
				...
				if ( $varId ) {
					...
					if ($productVar->get_image_id()) {
						return $productVar->get_image($image_size);
					}
				}
			}
		}
	 ...
	 * @return array
	 */
	public function getSearchTextSQL( $attribute, $value ) {
		global $wpdb;
		$clauses = array();
		if ( empty( $value ) ) {
			return $clauses;
			...
			foreach ($taxList as $key => $taxName) {
				if (strpos($attrs, $key) !== false) {
					...
					$where    .= '(' . $wpdb->prepare(" wpf_term_search.name %1s '%2s'", ( $word ? 'REGEXP' : 'LIKE' ), $searchValue) .
						' AND wpf_tax_search.taxonomy' . ( 'pa_attr' == $taxName ? " LIKE 'pa\_%'" : "='" . $taxName . "'" ) . ')';
				}
			}
			if ($taxSearch) {
		...
		if (strpos($attrs, 's') !== false) {
			...
			if ($metaKeyId) {
				...
				if ( $word ) {
					$where .= $wpdb->prepare( ' wpf_meta_sku.meta_value REGEXP %s', $searchValue );
				} else {
					$where .= $wpdb->prepare( ' wpf_meta_sku.meta_value=%s', str_replace( '%', '', $searchValue ) );
				}
			}
		}
	...
	public function checkBeforeFiltersFrontendArgs( $args, $filterSettings = array(), $urlQuery = array() ) {
		...
		foreach ( $vars as $key => $value ) {
			...
			if ( strpos( $key, 'pr_search_' ) === 0 && ! is_null( $value ) && strlen( $value ) > 0 ) {
				$clauses = $this->getSearchTextSQL( $key, $value );
				if ( empty( $clauses ) ) {
					break;
				}
	...
	public function addOptions( $options ) {
		$opts = array_merge($options['general']['opts'], array(
			...
			'loader_icon' => array(
				...
				'def' => 'default|0', 'add_sub_opts' => array($this, 'getSettingsLoaderHtml')
			),
	...
	public function getMetaFieldType( $type, $key ) {
		...
		if ( ( 'select' == $acfType && ! empty( $acfSetting['multiple'] ) ) || 'checkbox' == $acfType ) {
			$type = 9;
		} else if ( 'number' == $acfType || 'true_false' == $acfType ) {
			$type = 2;
		} else {
			$type = 0;
		}
	...
	public function getCustomTerms( $terms, $slug, $args ) {
		...
		if (strpos($slug, $this->ctax_prefix) === 0) {
			...
			if ($metaKeyId) {
				return FrameWpf::_()->getModule('meta')->getModel('meta_values')->getMetaValueTerms($metaKeyId, array_merge($args, array('key3' => str_replace($this->local_prefix, '', $slug), 'key2' => 'local', 'key4' => '')));
			}
			...
			if ($metaKeyId) {
				...
				return FrameWpf::_()->getModule('meta')->getModel('meta_values')->getMetaValueTerms($metaKeyId, array('field' => $metaField, 'fbv' => !empty($args['wpf_fbv'])));
			}
	...
	public function addCustomAttributesSql( $termProducts, $args ) {
		...
		foreach ($args['taxonomies'] as $slug) {
			...
			} else if (strpos($slug, $this->local_prefix) === 0) {
				$key = str_replace($this->local_prefix, '', $slug);
				if (false === $localId) {
					$localId = $woofilterModule->getMetaKeyId('_product_attributes');
				}
				if ($localId) {
					if (empty($metaFields['local']) || !in_array($metaKeyId, $metaFields['local'])) {
						$metaFields['local'][] = $key;
					}
				} else {
					$oldCalc[] = $slug;
				}
			} else if (strpos($slug, $this->meta_prefix) === 0) {
				$key = str_replace($this->meta_prefix, '', $slug);
				$metaKeyId = $woofilterModule->getMetaKeyId($key);
				if ($metaKeyId) {
					$field = $woofilterModule->getMetaKeyId($key, 'field');
	...
	public function addAjaxCustomMetaQueryPro( $metaQuery, $setting, $filterSettings = array() ) {		
		...
		if ( 'wpfAttribute' === $setting['id']) {
			...
			} else if (strpos($setting['name'], $this->meta_prefix) === 0) {
			...
			} else if (strpos($setting['name'], $this->local_prefix) === 0) {
			...
			if ($keyId) {
				...
				$clauses = $woofilterModule->addWpfMetaClauses(array('keyId' => $keyId, 'isAnd' => $isAnd, 'values' => $terms, 'field' => $woofilterModule->getMetaKeyId($key, 'field'), 'isLight' => false));
			}
	...
	public function addCustomMetaQueryPro( $metaQuery, $data, $mode ) {
		...
		foreach ($data as $key => $param) {
			...
			if (0 === strpos($key, $this->acf_prefix)) {
				...
				$keyId = $woofilterModule->getMetaKeyId($key);
			} else if (0 === strpos($key, $this->meta_prefix)) {
			...
			} else if (0 === strpos($key, $this->local_prefix)) {
			...
			if ($keyId) {
				...
				$woofilterModule->addWpfMetaClauses(array('keyId' => $keyId, 'isAnd' => $isAnd, 'values' => $isAnd ? $idsAnd : $idsOr, 'field' => $woofilterModule->getMetaKeyId($key, 'field'), 'isLight' => 'preselect' == $mode));
			}
	...
	public function addVariationQueryPro( $clauses, $urlQuery ) {
		...
		if (!empty($vars)) {
			...
			foreach ($vars as $key => $param) {
				...
				if (0 === strpos($key, $this->meta_prefix)) {
					...
					if ($keyId) {
						...
						foreach ($values as $val) {
							$i++;
							$clauses['join'] .= ' LEFT JOIN ' . DbWpf::getTableName('meta_data') . ' AS md' . $i . ' ON (md' . $i . '.product_id=p.ID AND md' . $i . '.key_id=' . $keyId . ')';
							$w .= ( empty($w) ? '' : ' AND ' ) . ' md' . $i . '.val_' . $field . ( $isAnd ? '=' . $val : ' IN (' . implode(',', $values) . ')' );
					...
					if ($keyId && $localId) {
						...
						if (!empty($valueIds)) {
							...
							$i++;
							if ($isAnd) {
								$clauses['join']   .= ' LEFT JOIN ' . $metaDataTable . ' md' . $i . ' ON (md' . $i . '.product_id=p.ID AND md' . $i . '.key_id=' . $localId . ' AND md' . $i . '.val_id=' . $leerId . ')';
								$clauses['having'] .= ( empty( $clauses['having'] ) ? '' : $whAnd ) . ' (count(DISTINCT md' . $i . '.val_id)>0';

								$i ++;
								$clauses['join']   .= ' LEFT JOIN ' . $metaDataTable . ' md' . $i . ' ON (md' . $i . '.product_id=p.ID AND md' . $i . '.key_id=' . $localId . ' AND md' . $i . '.val_id IN (' . implode( ',', $valueIds ) . '))';
								$clauses['having'] .= ' OR count(DISTINCT md' . $i . '.val_id)>=' . count( $valueIds ) . ')';

							} else {
								$valueIds[] = $leerId;
								$clauses['join'] .= ' LEFT JOIN ' . $metaDataTable . ' md' . $i . ' ON (md' . $i . '.product_id=p.ID AND md' . $i . '.key_id=' . $localId . ')';
				...
				if (!empty($w)) {
					$clauses['where'] .= ( empty($clauses['where']) ? '' : $whAnd ) . '(' . $w . ')';
				}
			}
		}
		$clauses['i'] = $i;
	...
	public function productLoopStart( $html, $settings = array(), $getGet = array() ) {
		...
		if ( $displayDescription ) {
			...
			foreach ( $getGet as $key => $value ) {
				if ( strpos( $key, 'filter_pwb' ) !== false && strpos( $value, '|' ) === false ) {
	...
	public function addCustomMetaKeys( $metaKeys, $filters ) {
		foreach ($filters as $filter) {
			if ('wpfAttribute' == $this->getFilterSetting($filter, 'id')) {
	...
	public function getColorGroup( $ids, $param, $filterId ) {
		...
		preg_match( '/group_(.*?)(?:_\d+)?_([^_]+?)$/', $param, $matches );
		if ( ! isset( $matches[2] ) ) {
			return $ids;
		}
	...
	public function getColorParents( $existTerms, $filters ) {

		if ( ! is_array( $existTerms ) || ! is_array( $filters ) ) {
			return $existTerms;
		}

		foreach ( $filters as $filter ) {
			if ( isset( $filter['settings']['f_frontend_type'] ) && 'colors' === $filter['settings']['f_frontend_type'] ) {
				$keyTerms = str_replace( 'filter_', 'pa_', $filter['name'] );

				if ( isset( $existTerms[ $keyTerms ] ) ) {
					$keysAll        = array_keys( $filter['settings'] );
					$keysColorGroup = preg_grep( '/^f_cglist\[\d+\]\[\]$/', $keysAll );
					...
					foreach ( $keysColorGroup as $key ) {
						...
						if ( '' !== $value ) {
							$children    = explode( ',', $value );
							$exists      = array_keys( $existTerms[ $keyTerms ] );
							$intersectAr = array_intersect( $exists, $children );
							if ( ! empty( $intersectAr ) ) {
								preg_match( '/^f_cglist\[(\d+)\]\[\]$/', $key, $parent );
								if ( isset( $parent[1] ) ) {
									$existTerms[ $keyTerms ][ $parent[1] ] = 1;
								}
							}
						}
					}
